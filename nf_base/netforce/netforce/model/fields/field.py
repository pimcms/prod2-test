...
class Field(object):
    ...
    def __init__(self, string=None, required=None, function=None, function_multi=False, function_context=None, function_order=None, store=None, on_change=None, on_write=None, translate=False, readonly=False, function_search=None, search=False, index=False, multi_company=False, agg_function=None, sql_function=None, function_write=None, help=None, unique=False):
        self.name = None
        self.model = None
        self.string = string
    ...
    def update_db(self):
        ...
        res = db.get(
            "SELECT * FROM pg_attribute a,pg_type t WHERE attrelid=(SELECT pg_class.oid FROM pg_class JOIN pg_catalog.pg_namespace n ON n.oid=pg_class.relnamespace WHERE relname=%s) AND attname=%s and t.oid=a.atttypid", m._table, self.name)
        if not res:
            ...
            if old_col_type != col_type:
                try:
                    ...
                    if col_type=="int4": # XXX
                        q+=" USING \"%s\"::integer"%self.name
                    db.execute(q)
                except:

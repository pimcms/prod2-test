    ...
    def get_report_data(self, ids, context={}):
        ...
        def get_min_qty(prod_id,min_months):
            ...
            while n<min_months:
                ...
                n+=1
            return total_qty
        def get_days_remain(prod_id,qty_avail,min_qty):
            sale_qtys=prod_sales.get(prod_id,{})
            n=0
            total_qty=0
            d=datetime.today()+relativedelta(day=31)
        ...
        for prod in get_model("product").search_browse(cond):
            ...
            min_qty=prod.min_qty or 0
            days_remain=get_days_remain(prod.id,qty_avail,min_qty)
            lead_days=prod.purchase_lead_time or 0
            need_order=days_remain<=lead_days
            if need_order:
                max_qty=prod.max_qty or 0
                order_qty=max(0,max_qty-min_qty)
            else:
                order_qty=None
            line_vals={
                "prod_id": prod.id,

...
class Conv(Controller):
    _path = "/convert_to_docx"

    def get(self):
        try:
            print("convert_to_docx")
            ...
            os.system(cmd)
            path2="/tmp/report_web.docx"
            #cmd="unoconv -f docx %s"%path
            cmd="cd /tmp; libreoffice --headless --calc --convert-to docx report_web.html"
            os.system(cmd)
            time.sleep(5)

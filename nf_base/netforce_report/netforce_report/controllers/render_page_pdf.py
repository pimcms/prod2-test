...
from netforce_report import render_page_pdf_reportlab, render_page_pdf_web
import json
import time
import os

class Report(Controller):
    _path = "/render_page_pdf"

    def get(self):
        try:
            print("render_page_pdf")
            ...
            ids_s=self.get_argument("ids",None)
            ids=json.loads(ids_s) if ids_s else None
            ...
            filename = self.get_argument("filename",None) or "report.pdf"
            props={}
            ...
            with database.Transaction():
                ...
                if page.type=="report":
                    res=render_page_pdf_reportlab(layout,props)
                elif page.type=="web":
                    res=render_page_pdf_web(page.path,active_id,ids,layout)
                else:

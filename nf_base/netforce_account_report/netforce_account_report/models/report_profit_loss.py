...
class ReportProfitLoss(Model):
    ...
    _fields = {
        ...
        "compare_with": fields.Selection([["month", "Previous Month"], ["quarter","Previous Quarter"], ["year", "Previous Year"]], "Compare With"),
    ...
    def get_report_data(self, ids=None, context={}):
        ...
        parent_acc_id=params.get("account_id")
        ...
        cond=[["type", "in", pl_types]]
        if parent_acc_id:
            cond.append(["id","child_of",parent_acc_id])
        res = get_model("account.account").search_read(cond, ["code", "name", "balance", "parent_id", "type", "hide_details"], order="code", context=ctx)
        accounts = {}
        parent_ids = []
        for r in res:
            accounts[r["id"]] = r
                ...
                if acc["type"] == "view":
                    ...
                    if children:
                        group = {
                            ...
                            "children": children,
                            "id": acc["id"],
                            "hide_details": acc.get("hide_details"),
                        }
                        groups.append(group)
        ...
        def _hide_details(group):
            if not group.get("children"):
                return
            if group.get("hide_details") and group["id"]!=parent_acc_id:
                group["children"]=[]
                return
            for c in group["children"]:
                _hide_details(c)
        _hide_details(income)
        _hide_details(cost_sales)
        _hide_details(other_income)
        _hide_details(expenses)
